...
static char help [] = 
HELP_HEAD("Linear Trajectory Model")
...
static char version [] =
VERSION_STRING("Linear Trajectory Model");
...
int main(int argc,  char *argv[])
{
        int i=1;
	char *fnout=0;
	PMProtein *evecs = pmCreateNew(PM_TYPE_MODEL);
	PMProtein *center = pmCreateNew(PM_TYPE_MODEL);
	PMProtein *out = pmCreateNew(PM_TYPE_MODEL);
	size_t idx=0,cframes=10;
	float peak=10.0;
	...
	while(i<argc){
				...
				SAY("Reference: %s ...",argv[i]);
                        	if(!pmReadRefM((PMProteinModel*)center,argv[i])){
					 WARN("Can not read \"%s\" <- will be ignored",argv[i]);
                        ...
                        if(argv[i+1][0]!='-'){
                                i++;
				SAY("Center: %s ...",argv[i]);
                        	if(!pmReadM((PMProteinModel*)center,argv[i])){
					 WARN("Can not read \"%s\" <- will be ignored",argv[i]);
				}
			}
                }
				...
				SAY("Eigenvector: %s ...",argv[i]);
                        	if(!pmReadM((PMProteinModel*)evecs,argv[i])){
					 WARN("Can not read \"%s\" <- will be ignored",argv[i]);
				}
			}
                }
                else if(!strcmp(argv[i],"-i") || !strcmp(argv[i],"--idxevec")){
                        if(argv[i+1][0]!='-')
                                idx=atol(argv[++i]);
                }
	...
	if(fnout){
		pmWriteM((PMProteinModel*)out,fnout);
	}
}
